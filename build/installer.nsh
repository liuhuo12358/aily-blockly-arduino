...
!macro customInstall
  ...
  nsExec::ExecToStack '"$INSTDIR\resources\app\child\7za.exe" x "$INSTDIR\resources\app\child\aily-builder-1.0.8.7z" -o"$INSTDIR\resources\app\child\aily-builder" -y'

  ; 删除解压后的压缩包，节省磁盘空间
  Delete "$INSTDIR\resources\app\child\aily-builder-1.0.8.7z"
  
  ; 等待解压完成
  Sleep 2000

  ; 删除解压后的压缩包，节省磁盘空间
  Delete "$INSTDIR\resources\app\child\aily-builder-1.0.0.7z"
